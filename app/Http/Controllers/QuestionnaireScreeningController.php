...
class QuestionnaireScreeningController extends Controller
{
    ...
    public function list(Request $request) : View
    {
        ...
        return view('pages.questionnaire-screening.list', $this->param);
    }

    public function screening($questionnaire) : View
    {
        $this->param['detail_questionnaire'] = $this->questionnaire->detail($questionnaire);
        $this->param['available_screening'] = $this->screening->availability_check(auth()->user()->id, $questionnaire);
        
        return view('pages.questionnaire-screening.screening', $this->param);
    }
    ...
    public function store(Request $request, $id) : RedirectResponse
    {
        ...
        $messages = [];
        $loop_iteration = 1;
        $generate_code = generate_screening_code();
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('questionnaire-screening.list')->with('success', "Berhasil melakukan Tes, silahkan cek hasilnya di menu <b>'Riwayat Tes'</b>. Kode Tes anda adalah: <b>".$generate_code."</b>");
