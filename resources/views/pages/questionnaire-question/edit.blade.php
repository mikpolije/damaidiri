<x-app-layout>
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <div class="px-3 pb-3">
                    <form method="POST" class="row g-3 mt-1 align-items-center" action="{{ route('master-questionnaire.questionnaire-question.update', [$detail_questionnaire->id, $detail_questionnaire_question->id]) }}">
                        @method('PUT')
                        @csrf
                        <div class="col-md-12">
                            <x-atoms.form-select-option id="questionnaire_category_id" name="questionnaire_category_id" label="Kategori Tes" class="select2" :options="array_merge(
                                $data_questionnaire_category->map(function($item) use ($detail_questionnaire_question) {
                                    return [
                                        'value' => $item->id, 
                                        'label' => $item->name, 
                                        'config' => old('questionnaire_category_id', $detail_questionnaire_question->questionnaire_category_id) == $item->id ? 'selected' : ''
                                    ];
                                })->toArray()
                                )" />
                        </div>
                        <div class="col-md-12">
                            <x-atoms.form-input id="name" label="Pertanyaan" required="true" type="text" name="name" value="{{ $detail_questionnaire_question->name }}" />
                        </div>
