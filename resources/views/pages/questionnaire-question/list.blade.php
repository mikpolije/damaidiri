<x-app-layout>
    ...
    @can('create_questionnaire_question')
        <div class="container-fluid bg-white rounded mt-4 shadow">
            <div class="row p-3">
                <div class="col-md-12">
                    <div class="px-3 pb-3">
                        <form method="POST" class="row g-3 mt-1 align-items-center" action="{{ route('master-questionnaire.questionnaire-question.store', $detail_questionnaire->id) }}">
                            @csrf
                            <div class="col-md-12">
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <form method="GET" action="{{ route('master-questionnaire.questionnaire-question.list', $detail_questionnaire->id) }}" class="row align-items-center p-3">
                    <div class="col-md-7">
            ...
            <div class="col-md-12">
                <div class="table-responsive px-3">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            @forelse ($data_questionnaire_question as $item)
                                <tr>
                                    <th scope="row" class="align-middle text-center fw-bold">{{ $loop->iteration + ($data_questionnaire_question->currentPage() - 1) * $data_questionnaire_question->perPage() }}</th>
                                    ...
                                    <td class="align-middle">
                                        <div class="widget-connection-request-actions d-flex">
                                            @can('update_questionnaire_question')
                                                <x-atoms.pure-button-redirect-parameter route="{{ route('master-questionnaire.questionnaire-question.edit', [$detail_questionnaire->id, $item->id]) }}" class="btn-primary btn-style-light flex-grow-1 m-r-xxs" icon="draw" label="Ubah" />
                                            @endcan
                                            @can('delete_questionnaire_question')
                                                <x-atoms.form-button-confirmation modal_name="modalDelete{{ $item->id }}" icon="delete" button_label="Hapus" class="btn-danger btn-style-light flex-grow-1 m-l-xxs" message_title="Apakah anda yakin ingin menghapus data?" message_description="Data yang sudah dihapus tidak dapat dikembalikan kembali." route="{{ route('master-questionnaire.questionnaire-question.destroy', [$detail_questionnaire->id, $item->id]) }}" method="DELETE" />
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center px-3 pb-3">
                    <div>
                        <span>Menampilkan <b>{{ number_format($data_questionnaire_question->count(), 0, ',', '.') }}</b> data dari <b>{{ number_format($data_questionnaire_question->total(), 0, ',', '.') }}</b> data.</span>
                    </div>
                    <div>
                        {{ $data_questionnaire_question->appends(request()->query())->links('components.molecules.table-pagination') }}
                    </div>
                </div>
            </div>
        </div>
