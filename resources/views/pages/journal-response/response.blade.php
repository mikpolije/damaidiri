<x-app-layout>
    <x-slot name="page_title"> Tes Jurnal - {{ $detail_journal->topic }} </x-slot>
    <x-slot name="page_description">
        <x-atoms.header-description icon="info" description="Silahkan lengkapi jurnal ({{ $detail_journal->topic }}) di bawah ini untuk melakukan pengisian jurnal." />
    </x-slot>
    <x-slot name="page_breadcrumb">
        <div class="table-responsive px-3">
            <table class="table table-bordered">
                ...
                <tr>
                    <td class="w-25 fw-bold bg-light text-black">Tujuan</td>
                    <td class="fw-bold text-center bg-light">:</td>
    ...
    <div class="container-fluid bg-white rounded mt-5 shadow">
        <form method="POST" class="row g-3 p-5" action="{{ route('journal-response.store', $detail_journal->id) }}">
            @csrf
            <div class="col-md-12">
            ...
            @if ($detail_journal->journal_question->count() > 0)
                <div class="col-md-12 text-end mt-4">
                    <x-atoms.form-button type="submit" class="btn-primary py-2" icon="compare" label="Simpan" />
                </div>
            @endif
        </form>
    </div>
