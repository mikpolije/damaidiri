<x-app-layout>
    <x-slot name="page_title">Daftar Jurnal</x-slot>
    <x-slot name="page_description">
        <x-atoms.header-description icon="info" description="Menu <b>'Daftar Jurnal'</b> berisi keseluruhan data jurnal yang tersedia di sistem dan dapat melakukan tes." />
    </x-slot>
    <x-slot name="page_breadcrumb">
        <x-atoms.breadcrumb :links="[
            ['name' => 'Dasbor', 'url' => route('redirect.auth')],
            ['name' => 'Daftar Jurnal', 'url' => route('journal-response.list')],
            ['name' => 'Jurnal', 'url' => route('journal-response.list')],
        ]" />
    </x-slot>
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <form method="GET" action="{{ route('journal-response.list') }}" class="row align-items-center p-3">
                    <div class="col-md-7">
                    ...
                    <table class="table table-bordered table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-white text-center fw-bold align-middle w-50" style="background-color: #21336e;">Tujuan</th>
                        ...
                        <tbody>
                            @forelse ($data_journal as $item)
                                <tr>
                                    ...
                                    <td class="align-middle w-25">{{ $item->topic }}</td>
                                    <td class="align-middle w-50">{{ $item->purpose }}</td>
                                    <td class="align-middle">
                                        <div class="row g-3 justify-content-center align-items-center">
                                            @can('response_journal')
                                                <div class="col-12 text-center">
                                                    <x-atoms.pure-button-redirect-parameter route="{{ route('journal-response.response', $item->id) }}" class="btn-dark btn-style-light" icon="display_settings" label="Isi Jurnal" />
                                                </div>
                                            @endcan
                                        </div>
                                    </td>
