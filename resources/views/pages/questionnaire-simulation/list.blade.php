<x-app-layout>
    <x-slot name="page_title"> Simulasi Tes </x-slot>
    <x-slot name="page_description">
        <x-atoms.header-description icon="info" description="Menu <b>'Simulasi Tes'</b> berisi keseluruhan data master Tes yang tersedia di sistem dan dapat melakukan simulasi." />
    </x-slot>
    <x-slot name="page_breadcrumb">
        <x-atoms.breadcrumb :links="[
            ['name' => 'Dasbor', 'url' => route('redirect.auth')],
            ['name' => 'Simulasi Tes', 'url' => route('simulation-questionnaire.list')],
            ['name' => 'Daftar Tes', 'url' => route('simulation-questionnaire.list')],
        ]" />
    </x-slot>
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <form method="GET" action="{{ route('simulation-questionnaire.list') }}" class="row align-items-center p-3">
                    <div class="col-md-7">
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <div class="row g-3 justify-content-center align-items-center">
                                            @can('simulate_simulation_questionnaire')
                                                <div class="col-12 text-center">
                                                    <x-atoms.pure-button-redirect-parameter route="{{ route('simulation-questionnaire.simulate', $item->id) }}" class="btn-dark btn-style-light" icon="display_settings" label="Simulasikan" />
                                                </div>
                                            @endcan
                                        </div>
                                    </td>
