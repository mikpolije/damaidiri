<x-app-layout>
    <x-slot name="page_title"> Daftar Master Jurnal </x-slot>
    <x-slot name="page_description">
        <x-atoms.header-description icon="info" description="Menu <b>'Daftar Master Jurnal'</b> berisi keseluruhan data master jurnal yang tersedia di sistem." />
    </x-slot>
    <x-slot name="page_action">
        @can('create_journal')
            <x-atoms.pure-button-redirect route="master-journal.create" class="btn-primary" icon="library_add" label="Tambah Data" />
        @endcan
    </x-slot>
    <x-slot name="page_breadcrumb">
        <x-atoms.breadcrumb :links="[
            ...
            ['name' => 'Daftar Master Jurnal', 'url' => route('master-journal.list')],
        ]" />
    </x-slot>
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <form method="GET" action="{{ route('master-journal.list') }}" class="row align-items-center p-3">
                    <div class="col-md-7">
                    ...
                    <table class="table table-bordered table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-white text-center fw-bold align-middle w-25" style="background-color: #21336e;">Tujuan Jurnal</th>
                        ...
                        <tbody>
                            @forelse ($data_journal as $item)
                                <tr>
                                    ...
                                    <td class="align-middle text-center">{{ $item->journal_question->count() }} Pertanyaan</td>
                                    <td class="align-middle">
                                        <div class="row g-3">
                                            @can('config_journal')
                                                <div class="col-6">
                                                    <x-atoms.pure-button-redirect-parameter route="{{ route('master-journal.config', $item->id) }}" class="btn-dark btn-style-light w-100" icon="display_settings" label="Konfigurasi" />
                                                </div>
                                            @endcan
                                            @can('update_journal')
                                                <div class="col-6">
                                                    <x-atoms.pure-button-redirect-parameter route="{{ route('master-journal.edit', $item->id) }}" class="btn-primary btn-style-light w-100" icon="draw" label="Ubah" />
                                                </div>
                                            @endcan
                                            @can('read_journal')
                                                <div class="col-6">
                                                    <x-atoms.pure-button-redirect-parameter route="{{ route('master-journal.detail', $item->id) }}" class="btn-warning btn-style-light w-100" icon="preview" label="Lihat" />
                                                </div>
                                            @endcan
                                            @can('delete_journal')
                                                <div class="col-6">
                                                    <x-atoms.form-button-confirmation modal_name="modalDelete{{ $item->id }}" icon="delete" button_label="Hapus" class="btn-danger btn-style-light w-100" message_title="Apakah anda yakin ingin menghapus data?" message_description="Data yang sudah dihapus tidak dapat dikembalikan kembali." route="{{ route('master-journal.destroy', $item->id) }}" method="DELETE" />
                                                </div>
                                            @endcan
                                        </div>
                                    </td>
