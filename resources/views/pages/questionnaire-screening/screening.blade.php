<x-app-layout>
    <x-slot name="page_title"> Tes - {{ $detail_questionnaire->title }} </x-slot>
    <x-slot name="page_description">
        <x-atoms.header-description icon="info" description="Silahkan lengkapi Tes ({{ $detail_questionnaire->title }}) di bawah ini untuk melakukan tes." />
    </x-slot>
    ...
    @if ($available_screening->available)
        <div class="container-fluid bg-white rounded mt-5 shadow">
            <form method="POST" class="row g-3 p-5" action="{{ route('questionnaire-screening.store', $detail_questionnaire->id) }}">
                @csrf
                <div class="col-md-12">
                    ...
                    <div class="card widget widget-info">
                        <div class="card-body">
                            <div class="widget-info-container">
                                ...
                                <h5 class="widget-info-title">Maaf Anda Belum Bisa Melakukan Tes Ini.</h5>
                                <p class="widget-info-text m-t-n-xs">Anda belum memenuhi syarat untuk mengikuti tes. Harap tunggu hingga {{ \Carbon\Carbon::parse($available_screening->available_date)->translatedFormat('d F Y')}}</p>
                            </div>
                        </div>
                    </div>
                </div>
