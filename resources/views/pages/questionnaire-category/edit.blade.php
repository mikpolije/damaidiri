<x-app-layout>
    ...
    <div class="container-fluid bg-white rounded mt-4 shadow">
        <div class="row p-3">
            <div class="col-md-12">
                <div class="px-3 pb-3">
                    <form method="POST" class="row g-3 mt-1 align-items-center" action="{{ route('master-questionnaire.questionnaire-category.update', [$detail_questionnaire->id, $detail_questionnaire_category->id]) }}">
                        @method('PUT')
                        @csrf
                        <div class="col-md-12">
                            <x-atoms.form-input id="name" label="Nama Kategori" required="true" type="text" name="name" value="{{ $detail_questionnaire_category->name }}" />
                        </div>
